...
print("getting mocap")
with open("Fist_and_Relax01_Mocap.csv", "r") as f:
    r = csv.reader(f)
    for i in range(5):
        next(r)
    for row in r:
...
with open("thevalues.csv", "r") as f:
    r = csv.reader(f)
    i = 0
    for line in r:
        lines[i,:] = line
...
np.save('labeled_data/ang_train.npy', ang_train)
np.save('labeled_data/ang_test.npy', ang_test)
np.save('labeled_data/us_train.npy', us_train)
np.save('labeled_data/us_test.npy', us_test)
print('Data saved')
